...
def install_new_jobs():
    ...
    job = my_cron.new(command='python3  /home/pi/smarteye/handlers/smartgen/pm_remote_uploader.py', comment = 'pm_remote_upload')
    job.minute.every(2)
    
    job = my_cron.new(command='python3  /home/pi/smarteye//handlers/smartgen/dfm_handler.py', comment = 'dfm_logger')
    job.minute.every(1)
    
    job = my_cron.new(command='python3  /home/pi/smarteye//handlers/smartgen/remote_config_updater_handler.py', comment = 'remote_config_updater')
    job.minute.every(1)
    
##    job = my_cron.new(command='python3  /home/pi/smarteye/handlers/lcd_write_handler.py', comment = 'smartgen_lcd_handler')
##    job.minute.every(1)
